...
let getQuestions = async (prod_id) => {
  ...
  var questions = await SetQuestions.aggregate(
    [
      { $match: { product_id: prod_id.toString() } }
    ]
  )
  return questions[0]
...
let postQuestions  = async (data) => {
  console.log(data, "innsss")
  let counting = await Questions.countDocuments({}, function( err, count){
    return count
  })
 let questionInfo = {
    question_id: counting +1,
    question_body: data.body,
    question_date: Math.floor(new Date().getTime() / 1000),
    asker_name: data.name,
    question_helpfulness: 0,
    reported: false,
  }
  let updated = await SetQuestions.findOneAndUpdate({product_id: data.product_id.toString()}, {"$push": { "results": questionInfo } })
  console.log(updated, "updatedddd")
  var obj = {
    question_id: counting +1,
...
let AnswersGet = async (questionId) => {
  var answers = await SetAnswers.aggregate(
    [{ $match: { question: Number(questionId) } }
    ])
